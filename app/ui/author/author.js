...
export default function Author({ signedRequest }) {
    ...
    useEffect(() => {
        ...
        const callbacks = {
            ...
            readyListener: function () {
                ...
                authorApp.on("save:activity", function (e) {
                    const noItems =
                        authorApp.getActivity().data.items.length === 0;
                    if (noItems) {
                        alert("Please add items before you save the activity!");
                        e.preventDefault();
                        setIsSaved(false);
                    }
                });
                ...
                authorApp.on("save:activity:success", () => {
                    ...
                    setIsSaved(true);
                    setActivityTemplateId(authorApp.getActivity().reference);
                });
            },
        };
        ...
        return () => {
            ...
            authorApp.destroy()
            document.querySelectorAll('.lrn-qe-tooltip-wrapper').forEach(el => {
                el.remove()
            })
        };
    }, []);
        ...
        <div>
            <div className="flex justify-center">
                {isSaved ? (
                    ...
                    <button
                        disabled
                        className="cursor-not-allowed disabled:opacity-40 text-white bg-blue-500 active:bg-blue:700 rounded-lg p-3 my-3 mt-5"
                    >
                        Take this Assessment!
                    </button>
                )}
